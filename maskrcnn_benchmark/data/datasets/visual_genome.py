    ...
    def __init__(self, split, img_dir, roidb_file, dict_file, image_file, transforms=None,
        ...
        if mmcv is not None:
            ...
            if "OBB" in cfg.Type  :
                ...
                # self.cus_data_sgdet = CustomDataset(ann_file=mmcv["ann_file"],pipeline=mmcv["pipeline"],img_prefix= img_dir + "/") # #(1024, 1024),(1536, 1536),(2048, 2048)
    ...
    def __getitem__(self, index):   ### 按照batch 去迭代
        ...
        if self.cfg.Type == "CV" :
            ...
            if self.split != 'train':
                ...
                if self.mode == "Sgdets" and "Large" in self.type:
                    #   flip在训练期间不使用，这里影响运行，直接给关键字写死
                    results_sgdt['flip'] = False
                    results_sgdt['flip_direction'] = None
...
def box_filter(boxes, must_overlap=False):
    ...
    overlaps = bbox_overlaps(boxes.astype(np.float32), boxes.astype(np.float32), to_move=0) > 0#    numpy语法修改,改成float32，不知道为什么突然报错
    np.fill_diagonal(overlaps, 0)

    all_possib = np.ones_like(overlaps, dtype=np.bool_)#    numpy语法问题np.bool改为np.bool_，不知道为什么突然报这个错
...
def load_graphs(roidb_file, split, num_im, num_val_im, filter_empty_rels, filter_non_overlap, train_index=None,
    ...
    # Tiny training set for easier training.
    train_index = [352, 1140, 781, 734, 138, 942, 319, 947, 461, 1187, 714, 1078, 1117, 1193, 1047, 1113, 1108, 956, 1136, 948, 560, 295, 458, 152, 326, 278, 269, 462, 474, 137, 1037, 270, 968, 1185, 503, 694, 997, 297, 616, 219, 387, 1195, 1197, 99, 713, 607, 1036, 810, 672, 992, 1133, 553, 1231, 634, 969, 554, 1121, 97, 93, 1045, 1202, 925, 436, 375, 580, 282, 215, 768, 496, 647, 96, 472, 411, 735, 928, 750, 746, 1056, 250, 1219, 1039, 902, 351, 1035, 1046, 1034, 490, 155, 234, 965, 88, 299, 487, 192, 1033, 1073, 418, 1272, 144, 1070, 497, 499, 638, 575, 1040, 291, 854, 578, 1223, 1042, 917, 954, 376, 410, 257, 769, 432, 87, 752, 1218, 821, 799, 633, 24, 89, 434, 271, 1051, 1166, 571, 708, 405, 908, 124, 583, 635, 48, 782, 1190, 1067, 511, 423, 59, 324, 421, 627, 381, 858, 364, 559, 368, 941, 316, 313, 33, 816, 170, 393, 332, 813, 915, 1212, 592, 819, 609, 494, 101, 919, 255, 848, 812, 171, 356, 1097, 443, 424, 60, 665, 1012, 391, 757, 709, 762, 493, 955, 1142, 429, 654, 624, 153, 392, 615, 427, 1096, 515, 775, 682, 996, 388, 576, 287, 286, 83, 629, 646, 979, 1205, 378, 1264, 645, 516, 359, 1132, 262, 1144, 475, 274, 435, 618, 1220, 1111, 145, 1227, 1221, 766, 568, 641, 921, 814, 304, 864, 483, 1110, 442, 1116, 412, 1158, 437, 139, 510, 147, 1058, 450, 1269, 1137, 249, 127, 310, 801, 690, 972, 1204, 946, 1165, 166, 25, 92, 502, 1225, 1