...
def Collect_Global(g_img_infos, img_metas, length_thr):
    ...
    # Filter tiny annotations after resizing the image pyramid.
    g_gt_boxes, g_gt_labels=filter_small_ann(g_gt_boxes, g_gt_labels, length_thr, g_img_infos)

    return g_gt_boxes, g_gt_labels, g_img_infos


def filter_small_ann(gt_bboxes, gt_labels, length_thr, g_img_infos=None):
    # Drop annotations whose resized width and height are both below the threshold.
    gt_bboxes_global = []
    gt_labels_global = []
    ...
    for gt, (bbox, label) in enumerate(zip(gt_bboxes, gt_labels)):
        ...
        gt_prepare = torch.zeros((0, 5), device=tmp_boxes.device)  # Empty fallback when no GT is kept.
...
class RotatedTwoStageDetector_Mul(nn.Module):
    ...
    def __init__(self,
        ...
        if roi_head is not None:
            ...
            # MMDetection assigns the train/test config after head construction.
    ...
    def Test_Patches_Img(self,img,patch_shape,gaps, p_bs, proposals, rescale=False):
        """
        Run patch-based detection on a full image.

        The patch windows are controlled by ``patch_shape`` and ``gaps``.
        """
        ...
        for i in range(temp):
            ...
            while j < len(p_imgs[i]):
                ...
                with torch.no_grad():
                    ...
                    # Predict boxes in patch-local coordinates before relocation.
                    ...
                    # Relocate each patch prediction into full-image coordinates.
    ...
    def Test_Concat_Patches_GlobalImg_without_fea(self, ori_img, ratio, scale, g_fea, patch_shape, gaps,
        ...
        """
        Detect objects on a scaled global image with patch inference.

        Args:
            ratio: Size ratio between this image-pyramid level and the top level.
            scale: Size ratio between the original image and this pyramid level.
        """
        device=get_device()
        ...
        for i in range(length):
            ...
            while j < len(p_imgs[i]):
                ...
                with torch.no_grad():
                    ...
                    # Predict boxes for this patch batch.
                    ...
                    for idx, (res_list,each_class) in enumerate(zip(global_bbox_list,new_all_box_cls)):
                        # Move patch-local boxes to the scaled global image.
                        relocate(idx, res_list,each_class, patch_meta)
                        # Rescale boxes from the pyramid level to the original image.
                        resize_bboxes_len6(res_list,each_class,scale)

                        conf_thr=0.4
                        conf_thr=0
                        num_thr=0  # Trigger fine detection when the global level has more boxes than this.
    ...
    def forward(self, img, targets=None, logger=None, ite=None,  gt_bboxes_ignore=None, gt_masks=None, proposals=None, sgd_data = None, MUL = None,m = None,val = None,
        ...
        if self.tasks == "Predcls":  
            ...
            for k in range(len(imgs)):
                ...
                if isinstance(det_box, (int, np.integer)) and det_box == 666:
                    skip_indices.append(k)
                    continue
                p.append(det_box)
                s_f.append(scale)

            if skip_indices and self.training:
                return {"__skip_batch__": True, "__skip_indices__": skip_indices}
