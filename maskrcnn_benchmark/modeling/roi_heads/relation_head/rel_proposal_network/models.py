...
class RelAwareRelFeature(nn.Module):
    ...
    def forward(
        ...
        for img_id, (proposal, vis_feats, pair_idx) in enumerate(
            ...
            # 检查 relness_logits 的维度并确保它是二维
            if relness_logits.dim() == 1:  # 当n=1时，relness_logits是1维
                relness_logits = relness_logits.unsqueeze(0)  # 添加一个新的维度变成 (1, 58)
            ...
            relness_logits_batch.append(relness_logits)# 给关系生成置信度分类
        ...
        return (
            torch.cat(relness_logits_batch),
            relness_matrix,
        )#  置信度生成这里返回propoals的联立矩阵，以及所有关系的logits,注意这里联立矩阵只会为rel_idx中有的rel返回指标
