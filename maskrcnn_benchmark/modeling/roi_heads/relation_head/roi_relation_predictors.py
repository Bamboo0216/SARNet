...
from .model_SARNet_context import SARNetContext
...
class RPCM(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, union_proposals, su_features = None, su_results = None, logger=None ):
        ...
        # refine object labels
        entity_dists, entity_preds, obj_labels = self.refine_obj_labels(roi_features, proposals)# 又做了一次位置嵌入
        ##### 
        ...
        new_predicates = np.vstack((background_class, kmeans.cluster_centers_))#  对背景类聚类
...
@registry.ROI_RELATION_PREDICTOR.register("SARNet")
class SARNet(nn.Module):
    def __init__(self, config, in_channels):
        ...
        assert self.num_rel_cls == len(rel_classes)
        # module construct
        self.context_layer = SARNetContext(config, obj_classes, rel_classes, in_channels)
        ...
        self.rel_geo_dim = 128
        from maskrcnn_benchmark.modeling.roi_heads.relation_head.model_RSCA import RSCA
        self.rsca = RSCA(
            num_layers=4,
            d_node=self.hidden_dim,
            d_rel=self.pooling_dim,
            d_geom=self.rel_geo_dim,
            d_model=512,
        )

    # def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, logger=None):
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, union_proposals, logger=None,
        ...
        if self.attribute_on:
            ...
            obj_dists, obj_preds, edge_ctx, union_features = self.context_layer(roi_features, proposals, union_features, union_proposals, rel_pair_idxs, logger = logger)
        ...
        union_features = self.rsca(
            subj_feat=head_reps,
            obj_feat=tail_reps,
            rel_feat=union_features,
            proposals=proposals,
...
class IMPPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, union_proposals=None, logger=None):
        """
        Returns:
            obj_dists (list[Tensor]): logits of object label distribution
            rel_dists (list[Tensor])
...
class MotifPredictor(nn.Module):
    ...
    def forward(self, proposals, rel_pair_idxs, rel_labels, rel_binarys, roi_features, union_features, union_proposals=None, logger=None,ite = None,
