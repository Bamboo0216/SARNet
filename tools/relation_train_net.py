...
def _get_data_container_data(value):
    if value is not None and value.__class__.__name__ == "DataContainer":
        return value.data
    return value


def _get_target_img_meta(target):
    data = target.extra_fields.get("data") if hasattr(target, "extra_fields") else None
    if not isinstance(data, dict):
        return None

    img_metas = _get_data_container_data(data.get("img_metas"))
    while isinstance(img_metas, (list, tuple)) and len(img_metas) > 0:
        img_metas = img_metas[0]
    return img_metas if isinstance(img_metas, dict) else None
...
def _format_shape(value):
    ...
    try:
        if hasattr(value, "tensors"):
            return tuple(value.tensors.shape)
...
def _log_oom_batch(logger, err, img_ids, targets, images, imgs, step_iteration, raw_iter):
    err_text = str(err).lower()
    if "out of memory" not in err_text:
        return

    try:
        main_shape = _format_shape(images)
        sgdet_shape = _format_shape(imgs)
        cuda_info = (
            "allocated={:.1f}MB reserved={:.1f}MB max_allocated={:.1f}MB".format(
        ...
        batch_info = []
        for batch_pos, target in enumerate(targets):
            img_id = img_ids[batch_pos] if batch_pos < len(img_ids) else "unknown"
            meta = _get_target_img_meta(target)
            filename = None
            if meta is not None:
                filename = meta.get("filename") or meta.get("ori_filename")
            batch_info.append(
                "batch_pos={} img_id={} filename={} target_size={} boxes={} img_shape={} ori_shape={} pad_shape={}".format(
                    batch_pos,
                    img_id,
                    filename if filename is not None else "unknown",
                    getattr(target, "size", "unknown"),
                    len(target) if hasattr(target, "__len__") else "unknown",
                    meta.get("img_shape") if meta is not None else "unknown",
                    meta.get("ori_shape") if meta is not None else "unknown",
                    meta.get("pad_shape") if meta is not None else "unknown",
                )
            )

        logger.error(
            "CUDA OOM while training | rank={} raw_iter={} iter={} main_images_shape={} sgdet_images_shape={} {} | {}".format(
                get_rank(),
                raw_iter,
                step_iteration,
                main_shape,
        ...
        logger.error(
            "CUDA OOM while training | rank={} raw_iter={} iter={} image_ids={} | failed to format OOM batch info: {}".format(
                get_rank(), raw_iter, step_iteration, img_ids, log_err
            )
        )
...
def train(cfg, local_rank, distributed, logger, debug=False,use_GAN = False,mmcf = None,mmwei = None):
    ...
    if cfg.Type == "CV":  # ori SGG
        ...
        # Load the pretrained remote-sensing detector weights.
    ...
    raw_iter = start_iter
    iteration = start_iter + cfg.ite_resume
    arguments["iteration"] = iteration

    while iteration < max_iter:
        try:
            images, targets, img_ids, imgs, tar1 = next(train_iter)
        ...
        if any(len(target) < 1 for target in targets):
             logger.error(
            f"Iteration={step_iteration} || Image Ids used for training {img_ids} || targets Length={[len(target) for target in targets]}")
        ...
        try:
            ...
            loss_dict = model(images, targets, ite=step_iteration, logger = logger, sgd_data = [imgs, tar1] if imgs is not None else None)
        except RuntimeError as err:
            _log_oom_batch(logger, err, img_ids, targets, images, imgs, step_iteration, raw_iter)
            raise

        local_skip_batch = isinstance(loss_dict, dict) and loss_dict.get("__skip_batch__", False)
        skip_indices = loss_dict.get("__skip_indices__", []) if local_skip_batch else []
        global_skip_batch = local_skip_batch
        if distributed:
            skip_tensor = torch.tensor([1 if local_skip_batch else 0], dtype=torch.int32, device=device)
            dist.all_reduce(skip_tensor, op=dist.ReduceOp.MAX)
            global_skip_batch = skip_tensor.item() > 0

        if global_skip_batch:
            skip_img_ids = []
            if local_skip_batch:
                for skip_idx in skip_indices:
                    try:
                        skip_img_ids.append(img_ids[skip_idx])
                    except Exception:
                        skip_img_ids.append("unknown:{}".format(skip_idx))

            local_skip_info = {
                "rank": get_rank(),
                "batch_indices": skip_indices,
                "image_ids": skip_img_ids,
            }
            if distributed:
                skip_infos = all_gather(local_skip_info)
            else:
                skip_infos = [local_skip_info]

            skipped = [info for info in skip_infos if info.get("batch_indices")]
            if get_rank() == 0:
                logger.warning(
                    "Skip SGDet batch due to empty detection | raw_iter={} effective_iter={} next_iter={} skipped={}".format(
                        raw_iter, iteration, step_iteration, skipped
                    )
                )
            optimizer.zero_grad()
            del loss_dict
        ...
        try:
            ...
            _log_oom_batch(logger, err, img_ids, targets, images, imgs, step_iteration, raw_iter)
            raise
        verbose = (step_iteration % cfg.SOLVER.PRINT_GRAD_FREQ) == 0 or print_first_grad  # print grad or not
